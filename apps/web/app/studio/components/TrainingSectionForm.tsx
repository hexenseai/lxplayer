...
interface TrainingSectionFormProps {
  ...
  onDone?: () => void;
  initialType?: 'video' | 'llm_interaction' | 'llm_agent';
}
...
export function TrainingSectionForm({ trainingId, initialSection, onDone, initialType }: TrainingSectionFormProps) {
  ...
  const defaultValues = initialSection ? { 
    ...
    description: initialSection.description ?? null,
    script: initialSection.script ?? null,
    duration: initialSection.duration ?? 1,
    video_object: (initialSection as any).video_object ?? null,
    asset_id: initialSection.asset_id ?? null,
    order_index: initialSection.order_index ?? 1,
    type: (initialSection as any).type ?? "video",
    agent_id: (initialSection as any).agent_id ?? null,
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      ...
      const apiValues = {
        ...
        video_object: values.video_object || null,
        asset_id: values.asset_id || null,
        agent_id: values.agent_id || null,
        language: values.language || null,
        target_audience: values.target_audience || null,
        order_index: values.order_index ?? (isUpdate ? initialSection?.order_index ?? 0 : 0)
      };

      if (isUpdate) {
        await api.updateTrainingSection(trainingId, initialSection!.id, apiValues);
      } else {
        await api.createTrainingSection(trainingId, apiValues);
      }
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 gap-4 max-w-2xl">
      ...
      <div>
        ...
        >
          ...
          <option value="llm_interaction">💬 LLM Etkileşim</option>
      ...
      <div>
        <Label htmlFor="description">
          {sectionType === 'llm_interaction' ? 'Etkileşim Açıklaması' : sectionType === 'llm_agent' ? 'Agent Rolü ve Görevi' : 'Tanıtım Metni'}
        </Label>
        <textarea 
          id="description" 
          {...register('description')} 
          placeholder={sectionType === 'llm_interaction' ? 'LLM etkileşiminin açıklamasını yazın' : sectionType === 'llm_agent' ? 'Agent\'ın rolünü ve görevini tanımlayın' : 'Bölüm hakkında tanıtım metni yazın'}
          className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
          rows={3}
        />
      </div>
      ...
      {sectionType === 'llm_agent' && (
        <>
          ...
          <div>
            <Label htmlFor="script">Agent Ön Bilgisi</Label>
            <textarea 
              id="script" 
              {...register('script')} 
              placeholder="Agent'a verilecek ön bilgi ve bağlam bilgilerini yazın"
              className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              rows={5}
            />
          </div>
      ...
      {(sectionType === 'llm_interaction' || sectionType === 'llm_agent') && (
